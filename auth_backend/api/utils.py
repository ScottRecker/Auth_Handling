...
def send_activation_email(user):
    ...
    link = settings.SERVER_BASE_URL + reverse(
        "api:user_activation", kwargs={"token": oauth_token}
    )
...
def revoke_token(token):
    ...
    token.blacklist()  # creates record in database that the token is blacklisted
